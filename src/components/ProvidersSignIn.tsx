...
const ProvidersSignIn = ({ chosenProvider, isLoading, setChosenProvider, setIsLoading }: ProvidersSignInProps) => {

    const signInProvider = async (provider: SignInProviders) => {
        setIsLoading(true);

        if (provider === SignInProviders.CREDENTIALS) throw new Error("Invalid Provider.");
        ...
        try {
            ...
            toast({
                title: "Error",
                message: err?.message ?? "Failed authentication, check with your provider and try again.",
                type: "error",
            });
            }
        } finally {
